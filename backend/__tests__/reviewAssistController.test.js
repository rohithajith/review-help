...
describe('review assist controller handlers', () => {
  ...
  test('getComposeQuestions returns db compose questions payload', async () => {
    reviewAssistService.sanitizeComposeQuestions.mockReturnValue([
    ...
    expect(reviewAssistService.sanitizeComposeQuestions).toHaveBeenCalled();
    expect(reviewAssistService.getComposeQuestions).not.toHaveBeenCalled();
  });
  ...
  test('getComposeQuestions returns 503 when db compose questions are missing', async () => {
    reviewAssistService.sanitizeComposeQuestions.mockReturnValue(null);
    const req = {
      businessId: '2',
      db: {
        query: jest.fn(async () => ({
